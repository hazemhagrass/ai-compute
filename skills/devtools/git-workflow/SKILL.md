...
## Enforcing the convention mechanically

Hand-written discipline does not survive a second contributor. Enforce the
format with a hook, then enforce it again where it cannot be skipped.
...
A local hook enforces nothing on its own: any contributor can bypass it with
`git commit --no-verify`. Run the same check in CI, where it cannot be skipped:
...
- **Pin `scope-enum` to real scopes.** An open scope list drifts into
  `fix(stuff)` within a month.
- **Lint the PR title too when you squash-merge.** With squash merges the PR
  title becomes the commit subject, so linting only the individual commits
  checks text that never lands on the default branch.
- **Allow a revert escape hatch.** `git revert` generates `Revert "..."`
  subjects that fail a strict subject rule; `config-conventional` permits
  them, a hand-rolled regex usually does not.
- **Do not lint merge commits.** They are generated by the host, not written
  by a person.
